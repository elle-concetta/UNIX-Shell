...
void init_shell(void) {
    printf("UNIX Simple Shell Program\n");
...
void read_line(char *line) {
    ...
    // If the input string is "exit" or is NULL, then end program
...
void parse_command(char *input_string, char **argv, int *wait) {
    ...
    // If there is &, wait = 0, otherwise wait = 1
...
int is_redirect(char **argv) {
    ...
    while (argv[i] != NULL) {
        if (strcmp(argv[i], TOFILE_DIRECT) == 0 || strcmp(argv[i], APPEND_TOFILE_DIRECT) == 0 || strcmp(argv[i], FROMFILE) == 0) {
            // IO redirection string found
            return i;
        }
        // Increment i by one
        i = -~i;
    }
    // No IO redirection string found
    return 0;
}

// Function to check for pipe arguments in the argument array
...
int simple_shell_cd(char **argv) {
    if (argv[1] == NULL) {
        ...
        if (chdir(argv[1]) != 0) {
            perror("Error: Error when changing the process's working directory to PATH.");
        }
    }
    return 1;
}
...
void exec_command(char **args, char **redir_argv, int wait, int res) {
    // Check for matches with any built-in command, if found execute, otherwise continue
